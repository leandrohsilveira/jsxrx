...
function generateLocationKey(node, id) {
  assert(typeof node.start === "number" && typeof node.end === "number", "Node must have start and end properties")
  const locationString = `${id}:${node.start}:${node.end}`;
  return createHash('sha256').update(locationString).digest('hex').substring(0, 8);
}
